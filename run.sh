...
start_postgres() {
    ...
    db_profile=jpa
    db_container_id=$(docker run -d -p 5432:5432 ${docker_postgres_args} postgres:14.5)
...
start_mongo() {
    ...
    db_profile=mongodb
    db_container_id=$(docker run -d -p 27917:27017 mongo:3.4)
}
...
run_application() {
    ...
    gradle bootRun -Dspring.profiles.active=verify,${db_profile}
}
